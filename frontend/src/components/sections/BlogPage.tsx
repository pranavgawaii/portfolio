...
export const ShareModal: React.FC<{ post: BlogPost; onClose: () => void }> = ({ post, onClose }) => {
  ...
  return (
    <motion.div
      ...
      className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/50 backdrop-blur-[6px]"
      onClick={onClose}
    >
      <motion.div
        ...
        className="relative w-full max-w-[420px] bg-white dark:bg-[#141414] rounded-2xl border border-neutral-200/80 dark:border-neutral-800 shadow-2xl p-7"
        onClick={e => e.stopPropagation()}
      >
        <button
          onClick={onClose}
          className="absolute top-5 right-5 w-7 h-7 flex items-center justify-center rounded-full hover:bg-neutral-100 dark:hover:bg-white/8 text-neutral-400 transition-colors"
        >
          <X size={14} />
        </button>

        <h3 className="font-sans font-semibold text-[15px] text-neutral-900 dark:text-neutral-100 mb-1">Share this post</h3>
        <p className="text-sm text-neutral-400 dark:text-neutral-500 mb-6 italic">"{post.title}"</p>

        {/* Copy link */}
        <p className="text-[10px] font-semibold uppercase tracking-widest text-neutral-400 dark:text-neutral-600 mb-2.5">Copy link</p>
        <div className="flex gap-2 mb-6">
          <div className="flex-1 flex items-center px-3 h-10 rounded-xl border border-amber-300/60 dark:border-amber-700/40 bg-amber-50/60 dark:bg-amber-900/10 font-mono text-[11px] text-neutral-700 dark:text-neutral-300 truncate">
            {url}
          </div>
          <button
            onClick={copy}
            className="shrink-0 w-10 h-10 flex items-center justify-center rounded-xl border border-neutral-200 dark:border-neutral-700 hover:bg-neutral-50 dark:hover:bg-white/5 text-neutral-500 transition-all"
          >
            <AnimatePresence mode="wait" initial={false}>
              {copied
                ? <motion.div key="check" initial={{ scale: 0.6 }} animate={{ scale: 1 }} exit={{ scale: 0.6 }}><Check size={14} className="text-emerald-500" /></motion.div>
                : <motion.div key="copy" initial={{ scale: 0.6 }} animate={{ scale: 1 }} exit={{ scale: 0.6 }}><Copy size={14} /></motion.div>
              }
            </AnimatePresence>
          </button>
        </div>
        ...
        {/* Share on */}
        <p className="text-[10px] font-semibold uppercase tracking-widest text-neutral-400 dark:text-neutral-600 mb-3">Share on</p>
        <div className="flex gap-2.5">
          <a
            href={twitterUrl} target="_blank" rel="noopener noreferrer"
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-neutral-950 dark:bg-white text-white dark:text-black text-[13px] font-medium hover:opacity-85 transition-opacity"
          >
            <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.744l7.73-8.835L1.254 2.25H8.08l4.259 5.633 5.905-5.633Zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
            Twitter / X
          </a>
          <a
            href={linkedinUrl} target="_blank" rel="noopener noreferrer"
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-neutral-200 dark:border-neutral-700 text-[13px] font-medium text-neutral-700 dark:text-neutral-300 hover:bg-neutral-50 dark:hover:bg-white/5 transition-colors"
          >
